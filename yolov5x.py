...
class Yolov5x:
    ...
    #     # 1/4
    #     p2 = x = Conv(out_channels=128, kernel_size=3, stride=2, padding='same')(x)
    #     x = C3(out_channels=128, num_bottles=3)(x)
    #     # 1/8
    #     p3 = x = Conv(out_channels=256, kernel_size=3, stride=2, padding='same')(x)
    #     x = C3(out_channels=256, num_bottles=6)(x)
    #     # 1/16
    #     p4 = x = Conv(out_channels=512, kernel_size=3, stride=2, padding='same')(x)
    #     x = C3(out_channels=512, num_bottles=9)(x)
    #     # 1/32
    #     x = Conv(out_channels=1024, kernel_size=3, stride=2, padding='same')(x)
    #     x = C3(out_channels=1024, num_bottles=3)(x)
    #     p5 = x = SPPF(in_channels=1024, out_channels=1024, kernel_size=5)(x)
    #
    #     # head
    #     x = Conv(out_channels=512, kernel_size=1, stride=1)(x)
    #     # 1/16
    #     x = tf.keras.layers.UpSampling2D(size=(2, 2), interpolation='nearest')(x)
    #     x = Concat(dimension=3)([x, p4])
    #     p6 = x = C3(out_channels=512, num_bottles=3, shortcut=False)(x)
    #     x = Conv(out_channels=256, kernel_size=1, stride=1, padding='same')(x)
    #     # 1/8
    #     x = tf.keras.layers.UpSampling2D(size=(2, 2), interpolation='nearest')(x)
    #     x = Concat(dimension=3)([x, p3])
    #     p7 = x = C3(out_channels=256, num_bottles=3, shortcut=False)(x)
    #     # 1/16
    #     x = Conv(out_channels=256, kernel_size=3, stride=2, padding='same')(x)
    #     x = Concat(dimension=3)([x, p6])
    #     p8 = x = C3(out_channels=512, num_bottles=3, shortcut=False)(x)
    #     # 1/32
    #     x = Conv(out_channels=512, kernel_size=3, stride=2, padding='same')(x)
    #     x = Concat(dimension=3)([x, p5])
    #     p9 = C3(out_channels=1024, num_bottles=3, shortcut=False)(x)
    #
    ...
    def build_graph(self):
        ...
        inputs = tf.keras.Input(shape=self.image_shape, batch_size=self.batch_size)
        # backbone
        x = Conv(out_channels=80, kernel_size=6, stride=2, padding='same')(inputs)
        # 1/4
        ...
        x = SPPF(in_channels=1280, out_channels=1280, kernel_size=5)(x)

        # head
        p5 = x = Conv(out_channels=640, kernel_size=1, stride=1)(x)
        # 1/16
        x = tf.keras.layers.UpSampling2D(size=(2, 2), interpolation='nearest')(x)
        x = Concat(dimension=3)([x, p4])
        x = C3(out_channels=640, num_bottles=4, shortcut=False)(x)
        p6 = x = Conv(out_channels=320, kernel_size=1, stride=1, padding='same')(x)
        # 1/8
        x = tf.keras.layers.UpSampling2D(size=(2, 2), interpolation='nearest')(x)
        x = Concat(dimension=3)([x, p3])
        p7 = x = C3(out_channels=320, num_bottles=4, shortcut=False)(x)
        # 1/16
        x = Conv(out_channels=320, kernel_size=3, stride=2, padding='same')(x)
        x = Concat(dimension=3)([x, p6])
        p8 = x = C3(out_channels=640, num_bottles=4, shortcut=False)(x)
        # 1/32
        x = Conv(out_channels=640, kernel_size=3, stride=2, padding='same')(x)
        x = Concat(dimension=3)([x, p5])
        p9 = C3(out_channels=1280, num_bottles=4, shortcut=False)(x)
...
if __name__ == "__main__":
    ...
    yolo5x = Yolov5x(image_shape=(416, 416, 3),
                     batch_size=2,
                     num_class=30,
                     anchors_per_location=3)
    model = yolo5x.build_graph()
