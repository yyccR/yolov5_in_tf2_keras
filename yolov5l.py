...
class Yolov5l:
    ...
    def build_graph(self):
        ...
        # 1/8
        x = Conv(out_channels=256, kernel_size=3, stride=2, padding='same')(x)
        p3 = x = C3(out_channels=256, num_bottles=6)(x)
        # 1/16
        x = Conv(out_channels=512, kernel_size=3, stride=2, padding='same')(x)
        p4 = x = C3(out_channels=512, num_bottles=9)(x)
        # 1/32
        x = Conv(out_channels=1024, kernel_size=3, stride=2, padding='same')(x)
        x = C3(out_channels=1024, num_bottles=3)(x)
        x = SPPF(in_channels=1024, out_channels=1024, kernel_size=5)(x)

        # head
        p5 = x = Conv(out_channels=512, kernel_size=1, stride=1)(x)
        # 1/16
        x = tf.keras.layers.UpSampling2D(size=(2, 2), interpolation='nearest')(x)
        x = Concat(dimension=3)([x, p4])
        x = C3(out_channels=512, num_bottles=3, shortcut=False)(x)
        p6 = x = Conv(out_channels=256, kernel_size=1, stride=1, padding='same')(x)
        # 1/8
        x = tf.keras.layers.UpSampling2D(size=(2, 2), interpolation='nearest')(x)
        x = Concat(dimension=3)([x, p3])
        p7 = x = C3(out_channels=256, num_bottles=3, shortcut=False)(x)
...
if __name__ == "__main__":
    ...
    yolo5l = Yolov5l(image_shape=(416, 416, 3),
                     batch_size=2,
                     num_class=30,
                     anchors_per_location=3)
    model = yolo5l.build_graph()
